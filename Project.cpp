#include <GLFW/glfw3.h>
 ...
 int delay = 100000000; // delay in ns for visualisation
 int lineWidth = 10;
 ...
 bool move() {
     ...
     switch (dir) {
         case 'E':
             if ((pos + 1) % size != 0 && walls[pos + 1][0] == 1) {
                 pos += 1;
                 pathIndex++;
                 path[pos] = pathIndex;
                 return true;
         ...
         case 'S':
             if (pos < size * (size - 1) && walls[pos + size][1] == 1) {
                 pos += size;
                 pathIndex++;
                 path[pos] = pathIndex;
                 return true;
         ...
         default:
             break; // This will not be executed since dir is always set above
     }
 ...
 int draw() {
     ...
     window = glfwCreateWindow(800,800, "Maze Solver", NULL, NULL);
     if (!window)
     {
         glfwTerminate();
         return -1;
 ...
 glClearColor(30/255.0f, 30/255.0f, 30/255.0f, 1);
 
 // Update color settings for different states:
 if (position == pos) {
     glColor3f(0, 1, 1); // Cyan for current position
 } else if (position == start) {
     glColor3f(1, 165/255.0f, 0); // Orange for start position
 } else if (cpathIndex == -1) {
     glColor3f(1, 1, 0); // Yellow for path
 } else if (mazeBlocks[position][3] != 0) {
     glColor3f(0.5, 0.5, 0.5); // Gray for explored cells
 } else if (mazeBlocks[position][2] != 0) {
     glColor3f(0.2, 0.2, 0.8); // Dark Blue for calculated cells
 } else if (cpathIndex > 0) {
     glColor3f(1, 105/255.0f ,180/255.0f); // Pink for other cells
 } else {
     glColor3f(30/255.f ,30/255.f ,30/255.f ); // Dark gray for default cells 
 }
